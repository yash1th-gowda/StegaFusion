"""
StegaFusion 16 KB FINAL PRODUCTION API INTEGRATION TEST
...
SECRET_FILE = Path(
    "input/secret_data/scalability_test/secret_16kb.bin"
)
...
OUTPUT_DIR = Path(
    "output/video_spatial_multiframe_16kb_final_integration"
)

OUTPUT_VIDEO = (
    OUTPUT_DIR / "stego_16kb.mp4"
)
...
RECOVERED_FILE = (
    RECOVERED_DIR / "recovered_16kb.bin"
)
...
def main():
    ...
    if original_size != 16384:

        raise RuntimeError(
            "Expected a 16384-byte secret file."
        )
    ...
    if (
        ...
        print()
        print(
            "The original 16 KB secret survived:"
        )
        ...
        print(
            "RESULT: 16 KB INTEGRATION FAIL."
        )

        print("=" * 70)

        raise RuntimeError(
            "16 KB byte/hash validation failed."
        )
