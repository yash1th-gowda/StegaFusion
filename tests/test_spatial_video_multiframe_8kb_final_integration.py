"""
StegaFusion 8 KB FINAL PRODUCTION API INTEGRATION TEST
...
SECRET_FILE = Path(
    "input/secret_data/scalability_test/secret_8kb.bin"
)
...
OUTPUT_DIR = Path(
    "output/video_spatial_multiframe_8kb_final_integration"
)

OUTPUT_VIDEO = (
    OUTPUT_DIR / "stego_8kb.mp4"
)
...
RECOVERED_FILE = (
    RECOVERED_DIR / "recovered_8kb.bin"
)
...
def main():
    ...
    if original_size != 8192:
        ...
        raise RuntimeError(
            "Expected a 8192-byte secret file."
        )
    ...
    if (
        ...
        print()
        print(
            "The original 8 KB secret survived:"
        )
        ...
        print(
            "RESULT: 8 KB INTEGRATION FAIL."
        )

        print("=" * 70)

        raise RuntimeError(
            "8 KB byte/hash validation failed."
        )
